...
m2 <- ulam(
  alist(
    s ~ dbinom(n, p),
    logit(p) <- a[tank] + b[size, pred],
    a[tank] ~ dnorm(a_bar, sigma),
    matrix[size, pred]:b ~ normal(0, 1),
    a_bar ~ dnorm(0, 1.5),
    sigma ~ dexp(1)
  ),
  data = dat,
...
```{r}
dat$n_std <- standardize(dat$n)
m3 <- ulam(
  alist(
    s ~ dbinom(n, p),
    logit(p) <- a[tank] + b[size, pred] + c * n_std,
    a[tank] ~ dnorm(a_bar, sigma),
    matrix[size, pred]:b ~ normal(0, 1),
    a_bar ~ dnorm(0, 1.5),
    sigma ~ dexp(1),
...
The posterior distribution for $\sigma$ in the model that includes density is shifted slightly more to the left than the one from problem 2. 
...
m12_5 <- ulam(
  alist(
    ...
    bi <- bii + bia * action + bic * contact,
    c(ba, bii, bc, bia, bic) ~ dnorm(0, 0.5),

...
In the three predictor model ($A$, $F$, and $G$), the coefficient on $A$ cannot be interpreted causally because its influence on weight is confounded by conditioning on $F$ (it's a pipe). I am not sure how the coefficients of $F$ and $G$ ought to be interpreted in this situation.
...
s |>
  ...
  ggplot() +
  geom_density(aes(x = pred_doy), color = "red")
```

The model predicts nonsense for a March temperature of 9. The range of predicted values isn't even less than 365: it's between 630 and 665. 
